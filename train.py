...
parser.add_argument('--param_init', help="The scale of the normal distribution from which weights are initialized",
                    type=float, default=0.01)
parser.add_argument('--clip_epoch', help="The starting epoch to enable clip", type=int, default=1)
parser.add_argument('--clip', help="The norm2 threshold to clip, set it to negative to disable", type=float,
                    default=-1.0)
...
def train_epoch(opt, shared, m, optim, ema, data, epoch_id, sub_idx):
    ...
    for i in range(data_size):
        ...
        # forward pass
        output, delta_score, imitate_delta_score = m.forward(wv_idx1, wv_idx2, cv_idx1, cv_idx2)

        loss_imitation = cosine_similarity(torch.flatten(delta_score.detach(), 1),
                                           torch.flatten(imitate_delta_score, 1)).sum()

        # loss
        batch_loss = loss(output, y_gold) + loss_imitation
