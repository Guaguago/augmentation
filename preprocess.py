...
def process(opt):
    ...
    import pickle
    with open('preprocess.pickle', 'rb') as pickle_in:
        opt, word_indexer, all_word_indexer, label_indexer, num_valid, num_train, num_test = pickle.load(pickle_in)

    convert(opt, word_indexer, all_word_indexer, label_indexer, opt.sent1_val, opt.sent2_val, opt.label_val,
            opt.output + "-val.hdf5", num_valid)
    convert(opt, word_indexer, all_word_indexer, label_indexer, opt.sent1, opt.sent2, opt.label,
            opt.output + "-train.hdf5", num_train)
    convert(opt, word_indexer, all_word_indexer, label_indexer, opt.sent1_test, opt.sent2_test, opt.label_test,
            opt.output + "-test.hdf5", num_test)
...
def main(arguments):
    ...
    parser.add_argument('--vocabsize',
                        help="Size of source vocabulary, constructed by taking the top X most frequent words. Rest are replaced with special UNK tokens.",
    ...
    parser.add_argument('--max_seq_l', help="Maximum sequence length. Sequences longer than this are dropped.",
                        type=int, default=100)
    parser.add_argument('--output', help="Prefix of the output file names. ", type=str, default="data/snli_1.0/snli")
    parser.add_argument('--shuffle', help="If = 1, shuffle sentences before sorting (based on source length).",
                        type=int, default=1)
